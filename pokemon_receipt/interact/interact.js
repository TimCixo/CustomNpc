...
var RECEIPT_MARKER = "receipt_type: cubix_pokemon";
...
function interact(event) {
    ...
    if (isPokemonReceiptBook(held)) {
        exchangeReceiptToPokemon(player, held);
        return;
    }

    player.message("§7Hold cubixcobblemon:pokemon to receive a signed receipt.");
    player.message("§7Hold a signed receipt to restore the Pokemon back.");
}
...
function exchangePokemonToReceipt(player, npc, held) {
    ...
    var receipt = createReceiptBook(npc, pokemon);
    if (receipt == null || receipt.isEmpty()) {
        player.message("§cCould not create the receipt book.");
        return;
    }
    ...
    player.message("§aPokemon was exchanged for a signed receipt.");
}
...
function exchangeReceiptToPokemon(player, held) {
    ...
    player.message("§aReceipt was exchanged back into a Pokemon.");
}
...
function isPokemonReceiptBook(item) {
    if (item == null || item.isEmpty()) return false;
...
function createReceiptBook(npc, pokemon) {
    try {
        ...
        pages.add(Filterable.passThrough(Component.literal(buildReceiptPage1(data))));
...
function buildReceiptPage1(data) {
...
function buildReceiptPage3(data) {
    return "Held Item: " + data.heldItem + "\n"
        + "Moves: " + data.moves;
}
...
function joinStats(statsObject) {
    ...
    for (var i = 0; i < STAT_ORDER.length; i++) {
        var value = statsObject.get(STAT_ORDER[i]);
        values.push("" + value);
    }
    return values.join("/");
}
